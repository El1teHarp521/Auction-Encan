...
export function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8000/lots')
      .then(res => res.json())
      .then(data => {
        // ИСПРАВЛЕНО: Превращаем оба ID в строки (String), чтобы они точно совпали
        // Также проверяем, что data - это массив
        if (Array.isArray(data)) {
          const myLots = data.filter((l: any) => String(l.sellerId) === String(user.id));
          setLots(myLots);
        }
        setLoading(false);
      })
        ...
        {lots.map((lot) => (
          <Card key={lot.id} className="p-4 flex flex-col md:flex-row gap-4 items-center border-primary/5 hover:border-primary/20 transition-all">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex items-center justify-center md:justify-start gap-1 text-[10px] text-muted-foreground mb-3">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <Button onClick={() => navigate(`/seller/lot/${lot.id}/bids`)} variant="outline" className="flex-1 text-[10px] font-bold uppercase h-8">Ставки ({lot.bidsCount || 0})</Button>
